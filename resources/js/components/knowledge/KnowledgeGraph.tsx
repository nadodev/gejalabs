...
const fallbackNodes: Node[] = [
  ...
  { id: "next", label: "React", x: 50, y: 90, accent: "info" },
];
...
export function KnowledgeGraph({ items = [] }: { items?: KnowledgeGraphItem[] }) {
  ...
  const edges = nodes.slice(1).map((node) => [center.id, node.id] as [string, string]);
